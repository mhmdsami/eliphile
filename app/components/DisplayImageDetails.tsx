...
export default function DisplayImageDetails({
  ...
  return (
    <div className="flex flex-col gap-3 w-11/12 md:w-4/5 lg:w-3/5 xl:w-2/5 mx-auto">
      <div className="text-xl font-bold">Your Images</div>
      <div className="p-3 rounded-lg grid grid-cols-3 md:grid-cols-7 gap-4 bg-black-too items-center place-items-center font-bold">
      ...
      {images.map(({ id, title, description, url, views }) => (
        <div
          key={id}
          className="p-3 rounded-lg grid grid-cols-3 md:grid-cols-7 gap-4 bg-black-too items-center place-items-center"
        >
          <div className="col-span-1 md:col-span-2">{title}</div>
          <div className="col-span-3 hidden md:block">{description}</div>
          <div className="col-span-1">{views}</div>
